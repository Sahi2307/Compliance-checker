...
UPLOAD_DIR = "uploads"
REPORTS_DIR = "reports"
VECTORS_PATH = r"D:\mithun works\AICompliancechecker\mywork\compliance_checker\vectorstores\local_vectors.pkl"
...
async def advanced_document_analysis(file: UploadFile = File(...), query: str = "Comprehensive document analysis"):
    ...
    try:
        ...
        document_text = " ".join(search_results['Documentname'])
...
async def download_report(filename: str):
    ...
    """
    print(f"Attempting to download report: {filename}")
    report_path = os.path.join(REPORTS_DIR, filename)

    if os.path.exists(report_path):
        print(f"Report found at {report_path}. Returning file.")
